# scripts/build_naver_supply_engine_a234.py
# HSinvest A234 NAVER READ_HTML FIX
# A233 오류: pandas.read_html(html)가 HTML 문자열을 파일 경로로 오인
# 해결: StringIO(html)로 감싸서 테이블 파싱
...
DETAIL = DATA / "naver_supply_detail_a234.json"
PIPE = DATA / "full_pipeline_summary_a234.json"
...
def read_naver_frgn_table(code, max_pages=8):
    ...
    for page in range(1, max_pages + 1):
        ...
        try:
            ...
            # 핵심 수정: HTML 문자열을 파일경로로 오인하지 않게 StringIO 사용
            ...
            if hit is None:
                errors.append({"page": page, "error": "table_not_found", "columns": debug_cols[:5]})
                continue
            ...
            errors.append({"page": page, "error": str(e)[:500]})
...
def calc_supply_from_naver(code):
    ...
    inst_col = find_col(cols, ["기관 순매매", "기관"])
    ...
    return {
        ...
        "indicators": {
            ...
            "source": "naver_frgn_readhtml_stringio",
            "unit": "shares"
        },
        "errors": errors[:5],
        "rows": len(df),
...
def main():
    ...
    summary.update({
        ...
        "supplySource": "naver_finance_item_frgn_readhtml_stringio",
