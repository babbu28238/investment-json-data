# A419 Robust Real Data Engine
# 핵심: 최신 영업일 자동 탐색 → KOSPI/KOSDAQ 전체 종목 → 주가/차트/수급 pykrx 우선 → 네이버 보조
...
STATUS_OUT = DATA / "data_connection_status_a419.json"
DEBUG_OUT = DATA / "robust_real_data_debug_a419.json"
...
def si(x):
    try:
        return int(float(str(x).replace(",", "").replace("+", "").replace("원", "").strip()))
    except Exception:
        return 0
...
def get_universe():
    try:
        from pykrx import stock
        date, date_debug = latest_krx_date()
        rows = []
        ...
        if rows:
            return rows, f"pykrx:{date}", {"dateDebug": date_debug[:10], "count": len(rows)}
    except Exception as e:
...
def fetch_price_krx(code, date):
    try:
        ...
        if df is not None and len(df) > 0:
            ...
            if close > 0:
                return close, "connected", f"pykrx_ohlcv:{date}", "KRX OHLCV 종가를 연결했습니다.", []
    except Exception as e:
        return 0, "failed", "pykrx_ohlcv", f"KRX OHLCV 실패: {str(e)[:80]}", []
    return 0, "failed", "pykrx_ohlcv", "KRX OHLCV 종가가 없습니다.", []
    ...
    for url in [f"https://m.stock.naver.com/api/stock/{code}/basic", f"https://m.stock.naver.com/api/stock/{code}/integration", f"https://finance.naver.com/item/main.naver?code={code}"]:
        try:
            ...
            for pat in [r'"closePrice"\s*:\s*"([0-9,]+)"', r'"now"\s*:\s*"([0-9,]+)"', r'"tradePrice"\s*:\s*([0-9]+)', r'현재가\s*([0-9,]+)']:
                ...
                if m and si(m.group(1)) > 0:
                    return si(m.group(1)), "connected", url, "네이버 종목 가격 데이터를 연결했습니다.", debug
        except Exception as e:
...
def fetch_price(code, date):
    ...
    if p > 0:
        return p, st, src, reason, dbg
    return fetch_price_naver(code)

def fetch_chart_krx(code, date):
    try:
        from pykrx import stock
        ...
        if df is None or len(df) < 20:
            return "차트 지표를 충분히 수집하지 못했습니다.", 50, {}, "failed", "KRX OHLCV 20거래일 미만"
        closes = [int(x) for x in df["종가"].tail(60).tolist() if int(x) > 0]
        if len(closes) < 20:
            return "차트 지표를 충분히 수집하지 못했습니다.", 50, {}, "failed", "KRX 종가 배열 부족"
        ma5 = sum(closes[-5:]) / 5
        ...
        score, reasons = 50, []
        if last > ma5 > ma20:
            score += 20; reasons.append("현재가가 5일선 위이고 5일선이 20일선 위")
        if last > ma20:
        ...
        return f"KRX 기준 현재가 {last:,}원, MA5 {ma5:,.0f}, MA20 {ma20:,.0f}, MA60 {ma60:,.0f}. " + ", ".join(reasons), clamp(score), {"ma5": round(ma5), "ma20": round(ma20), "ma60": round(ma60), "last": last}, "connected", "KRX OHLCV로 이동평균을 계산했습니다."
    except Exception as e:
        return "차트 지표를 충분히 수집하지 못했습니다.", 50, {}, "failed", f"KRX 차트 실패: {str(e)[:100]}"
...
def fetch_supply_krx(code, date):
    debug=[]
    try:
        from pykrx import stock
        end=datetime.strptime(date,"%Y%m%d")
        ...
        for detail in [True, False]:
            try:
                ...
                if f or i or p:
                    return f,i,p,"connected",f"pykrx_trading_value:{start}-{date}:detail={detail}",debug
            except Exception as e:
                debug.append({"method":f"value_detail_{detail}","ok":False,"error":str(e)[:120]})
    except Exception as e:
        debug.append({"method":"pykrx_import","ok":False,"error":str(e)[:120]})
    return 0,0,0,"failed","pykrx",debug

def fetch_supply(code, date, report_items, report_summary):
    f,i,p,status,source,debug = fetch_supply_krx(code, date)
    if status=="connected":
        sc=supply_score(f,i,p,status)
        return f,i,p,sc,status,f"최근 5거래일 KRX 기준 외국인 {f:,}, 기관 {i:,}, 연기금 {p:,}을 반영했습니다.",source,debug
    ...
    if m:
        f=si(m.group(1)); sc=supply_score(f,0,0,"partial")
        return f,0,0,sc,"partial",f"KRX 수급 실패 후 네이버 거래원 정보의 외국계추정합 {f:,}을 부분 반영했습니다.","naver_broker_foreign_estimate",debug
...
def final_score(c):
    q=si(c.get("quantScore")) or 50; s=si(c.get("supplyScore")) or 50; co=si(c.get("companyScore")) or 50
    ev=si(c.get("eventScore")) or si(c.get("newsScore")) or 50; report=si(c.get("reportScore")) or 50
    chart=si(c.get("chartScore")) or 50; ma=si(c.get("macroScore")) or 50; ri=si(c.get("riskScore")) or 50
...
def infer_sector(name, market):
    sector=market
    keywords={
        "반도체":["삼성전자","SK하이닉스","이오테크닉스","한미반도체"],
        "조선":["조선","한화오션","HD현대중공업","HD한국조선해양"],
        "방산":["한화시스템","현대로템","한국항공우주","LIG넥스원"],
        "바이오":["바이오","셀트리온","알테오젠"],
        "증권":["증권"],
        "자동차":["현대차","기아"]
    }
    for k, arr in keywords.items():
        if any(x in name for x in arr):
            sector += " " + k
    return sector
...
def main():
    ...
    for idx,row in enumerate(universe):
        ...
        price,ps,psrc,preason,pdbg=fetch_price(code,date)
        if ps!="connected":
            counts["naverMissing"]+=1
            debug.append({"code":code,"name":name,"status":"price_failed","priceDebug":pdbg})
            continue
        ...
        chart,chart_sc,chart_ind,cs,creason=fetch_chart_krx(code,date)
        if cs=="connected": counts["chartConnected"]+=1
        f,i,p,supply_sc,ss,sreason,ssource,sdbg=fetch_supply(code,date,report_items,report)
        ...
        company=clamp(report_sc*.55+pscore*.45); event=news_sc; macro_sc=50; risk=60
